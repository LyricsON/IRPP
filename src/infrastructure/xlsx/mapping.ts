...
export function declarationPatches(input: TaxpayerInput, result: TaxResult): CellPatch[] {
  ...
  const out: CellPatch[] = [
    ...
    ...(result.parentDeductions === '0.000' ? [] : [{ sheet: declaration, cell: 'C263', value: result.parentDeductions, kind: 'number' as const }]),
    ...(input.deductions.universityLoanPaid === undefined ? [] : [{ sheet: declaration, cell: 'C264', value: input.deductions.universityLoanPaid, kind: 'number' as const }]),
    ...(input.deductions.nonSalariedSocialContributions === undefined ? [] : [{ sheet: declaration, cell: 'C265', value: input.deductions.nonSalariedSocialContributions, kind: 'number' as const }]),
    ...(input.deductions.housing && result.warnings.every(warning => !warning.includes('Prêt logement')) ? [{ sheet: declaration, cell: 'C266', value: input.deductions.housing.annualInterest, kind: 'number' as const }] : []),
  ...
  input.family.children.slice(0, 4).forEach((child, index) => {
    ...
    out.push({ sheet: detail, cell: childFlagCells[index], value: eligible ? '1' : '0', kind: 'number' })
  })
  split(input.identity.cin).forEach((digit, index) => out.push({ sheet: declaration, cell: cinCells[index], value: digit, kind: 'string' }))
  split(input.identity.postalCode).forEach((digit, index) => out.push({ sheet: declaration, cell: ['C23', 'D23', 'E23', 'F23'][index], value: digit, kind: 'string' }))
  if (birth.length === 3) {
    split(birth[0]).forEach((digit, index) => out.push({ sheet: declaration, cell: ['O21', 'P21', 'Q21', 'R21'][index], value: digit, kind: 'string' }))
    split(birth[1]).forEach((digit, index) => out.push({ sheet: declaration, cell: ['T21', 'U21'][index], value: digit, kind: 'string' }))
    split(birth[2]).forEach((digit, index) => out.push({ sheet: declaration, cell: ['W21', 'X21'][index], value: digit, kind: 'string' }))
  }
  return out
}
