...
function requireValidXml(document: Document, label: string) { if (document.documentElement.localName === 'parsererror') throw new Error(`XML invalide : ${label}`); return document }
function sheetPath(workbook: Document, relationships: Document, name: string) {
  const sheet = [...workbook.getElementsByTagNameNS(ns, 'sheet')].find(item => item.getAttribute('name') === name)
  const id = sheet?.getAttributeNS(relationshipsNs, 'id') || sheet?.getAttribute('r:id')
  const relationship = id ? [...relationships.getElementsByTagNameNS('http://schemas.openxmlformats.org/package/2006/relationships', 'Relationship')].find(item => item.getAttribute('Id') === id) : undefined
...
function requestFullRecalculation(workbookXml: string) {
  ...
  return workbookXml.replace('</workbook>', '<calcPr fullCalcOnLoad="1" forceFullCalc="1"/></workbook>')
}
function setCell(xml: string, cellRef: string, value: string, kind: 'string' | 'number', replaceFormula = false) {
  ...
  if (!openingTag || openingTag.index === undefined) throw new Error(`Cellule cible absente : ${cellRef}`)
  ...
  if (!replaceFormula && !isSelfClosing && /<f(?:\s|>)/i.test(xml.slice(openingEnd, closingStart))) throw new Error(`Formule protégée : ${cellRef}`)
  const body = kind === 'number' ? `<v>${xmlEscape(value)}</v>` : `<is><t>${xmlEscape(value)}</t></is>`
...
export async function generateDeclaration(input: TaxpayerInput, result: TaxResult) {
  if (result.status !== 'ready') throw new Error('Export indisponible tant que le calcul nécessite une vérification.')
  ...
  const workbook = requireValidXml(new DOMParser().parseFromString(workbookXml, 'application/xml'), 'classeur')
  const relationships = requireValidXml(new DOMParser().parseFromString(relationshipsXml, 'application/xml'), 'relations')
  const bySheet = new Map<string, CellPatch[]>()
  for (const patch of declarationPatches(input, result)) bySheet.set(patch.sheet, [...(bySheet.get(patch.sheet) ?? []), patch])
  for (const [sheet, patches] of bySheet) {
    const path = sheetPath(workbook, relationships, sheet); let xml = await zip.file(path)?.async('string')
    if (!xml) throw new Error(`XML absent : ${sheet}`)
    for (const patch of patches) if (patch.value !== null) xml = setCell(xml, patch.cell, patch.value, patch.kind, patch.replaceFormula)
    zip.file(path, xml)
  }
