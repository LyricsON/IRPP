...
const blank = (): TaxpayerInput => ({ year: 2026, unsupportedIncome: false, identity: { cin: '', fullName: '', birthDate: '', address: '', postalCode: '', profession: '' }, salaryBase: '0', professionalAbatement: '0', irppWithheld: '0', cssWithheld: '0', family: { chefEligible: false, children: [], parents: [] }, deductions: {} })
...
const demo = (): TaxpayerInput => ({
  ...
  family: {
    chefEligible: true,
    children: ['2010-01-01', '2011-01-01', '2012-01-01'].map((birthDate, index) => ({ id: `demo-child-${index + 1}`, birthDate, hasSeparateIncome: false, higherEducation: false, receivesScholarship: false, disabled: false })),
    parents: []
  },
  deductions: { nonSalariedSocialContributions: localValue('VITE_IRPP_TEST_NON_SALARIED_SOCIAL_CONTRIBUTIONS', '') || undefined }
})
...
export const useDeclarationStore = defineStore('declaration', () => {
  ...
  function reset() { data.value = blank() }
  function loadDemo() { data.value = demo() }
